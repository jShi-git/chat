...
class Event
{
   ...
   public static function onClose($client_id)
   {
       ...
       if(isset($_SESSION['room_id']))
       {
           ...
           {
               $client_list = self::formatClientsData($all_clients);
               $new_message = array('type'=>'logout', 'from_client_id'=>$client_id, 'from_client_name'=>$client_name, 'client_list'=>$client_list, 'client_name'=>htmlspecialchars($client_name), 'time'=>date('Y-m-d H:i:s'));
               $client_id_array = array_keys($all_clients);
               Gateway::sendToAll(json_encode($new_message), $client_id_array);
           }
       }
